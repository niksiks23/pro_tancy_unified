...
def main_menu():
    ...
    btn2 = telebot.types.KeyboardButton("ℹ️ Мои настройки")
...
def broadcast_start(message):
    ...
    if user_id not in ADMIN_IDS:
        ...
        return
    msg = bot.send_message(user_id, "📢 **Отправьте сообщение для рассылки всем пользователям**", parse_mode='Markdown')
...
def process_broadcast(message):
    ...
    if not users:
        ...
        return
    status_msg = bot.send_message(admin_id, f"📤 Начинаю рассылку **{len(users)}** пользователям...", parse_mode='Markdown')
    success, failed = 0, 0
    ...
    try:
        bot.edit_message_text(final_text, admin_id, status_msg.message_id, parse_mode='Markdown')
    except:
